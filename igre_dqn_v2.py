...
class IgreDQNTrainer:
    ...
    def actor_loss(
        ...
        if NON_LOG_PROB_PENALTY:
            ...
            m = torch.ones_like(cur_actor_logits).scatter(-1, labels.unsqueeze(-1), 0)
            ...
            tmp = (
                F.relu(masked_logits)
                * mask.unsqueeze(dim=2)
                * F.relu(y_i.unsqueeze(dim=2))
            )
            tmp_weight = 1e7
    ...
    def critic_loss(self, critic_values, expected_future_rewards):
        ...
        loss = F.mse_loss(critic_values, expected_future_rewards.view(-1, 1))
        return loss
