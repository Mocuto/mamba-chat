...
class iEncoderJamba(nn.Module):
    def __init__(
        self,
        d_model: int,
        n_vocab: int,
    ):
        super(iEncoderJamba, self).__init__()
        n_layer = 6
        heads = 8
        config = JambaConfig(
            vocab_size=n_vocab + 2,
        ...
        self.embedding_weight = self.mixer.get_input_embeddings().weight

    def forward(self, x, output_pooled_state=False):
        x = self.mixer(input_ids=x, output_hidden_states=True).last_hidden_state
        if output_pooled_state:
            s = x.mean(dim=1)
            return x, s
        return x
...
class iActor(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        use_jamba = kwargs.get("use_jamba", False)
        self.decoder = (
            iDecoder(d_model, n_vocab)
            if not use_jamba
            else iDecoderJamba(d_model, n_vocab)
        )
...
class iSys1(nn.Module):
    def __init__(self, d_model: int, n_vocab: int, **kwargs):
        super(iSys1, self).__init__()
        self.option_gen = iOptionGen(d_model)
        self.actor = iActor(d_model, n_vocab, **kwargs)
...
BIAS_WITH_CRITIC = True


class Igre(nn.Module):
    def __init__(self, d_model: int, n_vocab: int, tokenizer, use_jamba=False):
        super(Igre, self).__init__()
        self.encoder = (
            iEncoder(d_model, n_vocab)
            if not use_jamba
            else iEncoderJamba(d_model, n_vocab)
        )
        self.sys1 = iSys1(d_model, n_vocab, use_jamba=use_jamba)
...
if __name__ == "__main__":
    ...
    igre_jamba = Igre(32, 50265, tokenizer, use_jamba=True).cuda()
    result = igre_jamba.gen1(torch.tensor([1, 2, 3]).unsqueeze(0).cuda(), c=0)
    print(result)
