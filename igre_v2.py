...
REWARD_WEIGHT_MATRIX = torch.tensor([-1., -0.1, 0.1, 1.])
...
class iDecoderBasedCubed(nn.Module):
    def __init__(self, based_model, reward_size):
        super(iDecoderBasedCubed, self).__init__()
        self.d_model = based_model.embeddings.word_embeddings.weight.shape[1]
        self.vocab_size = based_model.embeddings.word_embeddings.weight.shape[0]
        self.reward_size = reward_size

        self.based_model = based_model if not DEEP_COPY_DECODER else copy.deepcopy(based_model)
        self.lm_head = nn.Linear(self.d_model, self.vocab_size * reward_size, bias=True)
    ...
    def forward(self, x, num_last_tokens=0):
        ...
        if num_last_tokens > 0:
            hidden_states = hidden_states[:, -num_last_tokens, :]
        lm_logit_cube = self.lm_head(hidden_states)
        lm_logit_cube = lm_logit_cube.view(-1, x.shape[1], self.reward_size, self.vocab_size)
        return lm_logit_cube
...
class iActor(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.decoder = iDecoderBasedCubed(based_model, kwargs.get("reward_size", 4))
...
class Igre(nn.Module):
    ...
    def _gen1_decode(self, input_ids, z, **kwargs):
        ...
        logit_cube_weights = kwargs.get("logit_cube_weights", REWARD_WEIGHT_MATRIX)

        scores, seqs = [], [input_ids]
        allowed_tokens = kwargs.get("allowed_tokens", [])

        def get_logits(input_ids, z):
            logit_cube = self.sys1_forward_with_option(
                input_ids, z, num_last_tokens=1, allowed_tokens=allowed_tokens
            )
            logits = (logit_cube * logit_cube_weights.view(logit_cube.shape[0], 1, 1, -1)).sum(dim=-1)
            return logits
    ...
    def sys1_forward_with_option(
        ...
        if BIAS_WITH_CRITIC:
            ...
            logit_cube = self.sys1.decode_option(x, z, num_last_tokens=num_last_tokens)
            # Of shape [batch_size, num_last_tokens, vocab_size, reward_size]
